...
- Linuxディストリビューションとは何か、およびその目的について説明してください。
   ...
   Linuxディストリビューションとは：Linuxカーネルと必要なアプリケーションやソフトウェアを1つにまとめ、利用者が簡単にインストールして使えるようにしたパッケージ
...
ユーザーからのコマンドを解釈してカーネルに渡し、OSに命令を実行させるインターフェースの役割を果たしています.
...
bin：一般ユーザーおよびシステムが利用する基本的なコマンド（例: ls, cp など）を置くディレクトリ
dev：ハードウェア機器（デバイス）を表す特殊なファイル,
etc：システム全体・各種プログラムの設定ファイルをおくディレクトリ
home：ユーザーのホームディレクトリ
sbin：システム管理者やルートユーザー向けのコマンドを置くディレクトリで、一般ユーザーは実行しないコマンドが含まれる
tmp：一時的なファイルを保存
usr：複数のマシンに跨って共有可能なファイルを置くディレクトリ
var：ログファイルやキャッシュなど、サイズが変わる可能性のあるデータを保存するディレクトリ、あとから削除されるデータ
...
ファイルの関連性：リンク自体は独立したエンティティであり、リンク先のファイルまたはディレクトリへ参照情報のみを保持。リンク先が存在しない場合は無効になる。
