...
専門家が質問に回答するQ&Aサービス。
ユーザーと専門家を結びつけ、知識共有を促進することを目的とした自社サービスです。
...
スマートフォンの限られた画面でもユーザーが直感的に操作できるよう、視線誘導を意識したレイアウト設計を行いました。
また、Figmaのコンポーネント機能を活用し、効率的で保守性の高いデザインデータを作成しました。

実装では、CSS Flexboxを用いて1px単位でデザインを忠実に再現。ボタンやフォームなどを再利用可能なコンポーネントとして設計することで、
保守性と開発生産性を両立するコーディングスキルを習得しました。インタラクティブなUI（ホバーやクリック時のアニメーション）の実装も経験しました。
...
#### 取り組んだこと
未経験から膨大な専門用語や技術概念を学ぶ上で、知識の定着に苦労しました。
インプットした知識を自分なりにドキュメントへまとめたり、簡単なコードを書いて動かしてみたりと、アウトプットをセットで行う学習サイクルを徹底しました。
効率的な学習方法を確立し、未経験からでも新しい技術へ主体的にキャッチアップしていく自信を得ました。
